...
env -u LD_LIBRARY_PATH CUDA_VISIBLE_DEVICES=${BEHAVIOR_GPU:-1} "$PY" scripts/eval_webmt_behavior_dev.py \
  ...
  --data-dir "$DATA" \
  --output-dir results/63_webmt_behavior/base_grpo400 \
  --batch-size 4

env -u LD_LIBRARY_PATH CUDA_VISIBLE_DEVICES=${BEHAVIOR_GPU:-1} "$PY" scripts/eval_webmt_behavior_dev.py \
  --model-path outputs/62_webmt_lora_v1_merged \
  --data-dir "$DATA" \
  --output-dir results/63_webmt_behavior/webmt_lora_v1 \
  --batch-size 4

echo WEBMT_BEHAVIOR_COMPARE_PASS
