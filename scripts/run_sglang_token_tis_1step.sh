#!/usr/bin/env bash
# Step B: 1-step SGLang + official Decoupled Token-TIS. Does not edit 07_run_evidence_grpo.sh.
set -euo pipefail
...
LR=${GRPO_LR:-1e-8}
OUT=${OUT:-"$PROJECT_ROOT/results/35_sglang_token_tis_1step"}
...
VIEW="$OUT/model_view"
C_VIEW="$C_DEE/results/35_sglang_token_tis_1step/model_view"
...
echo "STEP_B batch=$BATCH n=$N mini=$MINI lr=$LR tis=token/2.0"
set +e
docker exec \
  ...
  trainer.experiment_name=sglang_token_tis_1step \
  trainer.default_local_dir="$C_DEE/results/35_sglang_token_tis_1step/ckpt" \
  2>&1 | tee "$LOG"
rc=${PIPESTATUS[0]}
set -e
docker exec "$CONTAINER" bash -c 'ray stop --force 2>/dev/null; pkill -f sglang.launch_server 2>/dev/null; exit 0' || true
echo "STEP_B_EXIT=$rc log=$LOG"
exit "$rc"
