...
if [ "$accepted" -eq 0 ]; then
  echo "SOURCE_FIRST_V2_NO_CANDIDATES synthesis_exit=$synthesis_rc"
  exit 1
fi
echo "SOURCE_FIRST_V2_SUBSET accepted=$accepted synthesis_exit=$synthesis_rc"
...
audit_rc=$?
echo "SOURCE_FIRST_V2_BUILD_EXIT=$build_rc AUDIT_EXIT=$audit_rc"
